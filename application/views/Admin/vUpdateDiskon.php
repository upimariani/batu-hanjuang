			...
			<div class="tm-bg-primary-dark tm-block tm-block-h-auto">
				...
				<div class="row tm-edit-product-row">
					<div class="col-xl-12 col-lg-12 col-md-12">
						<form action="<?= base_url('Admin/cDiskon/update/' . $tiket->id_tiket) ?>" method="POST" class="tm-edit-product-form">
							<div class="form-group mb-3">
								...
								<select class="custom-select tm-select-accounts" name="nama" id="category">
									...
									?>
											<option value="<?= $value->id_tiket ?>" <?php if ($value->id_tiket == $tiket->id_tiket) {
																						echo 'selected';
																					} ?>><?= $value->nama_tiket ?></option>
									<?php
										}
									}
									?>
							...
							<div class="form-group mb-3">
								...
								<input name="tgl" value="<?= $tiket->tgl_diskon ?>" type="date" class="form-control validate" />
								<?= form_error('tgl', '<small class="text-danger">', '</small>') ?>
							</div>
							<div class="form-group mb-3">
								<label for="description">Nama Diskon</label>
								<input id="name" name="nama_diskon" value="<?= $tiket->nama_diskon ?>" type="text" class="form-control validate" />
								<?= form_error('nama_diskon', '<small class="text-danger">', '</small>') ?>
							</div>
							<div class="form-group mb-3">
								<label for="description">Besar Diskon</label>
								<input id="name" name="besar" value="<?= $tiket->diskon ?>" type="number" class="form-control validate" />
							...
							<div class="form-group mb-3">
								...
								<select class="custom-select tm-select-accounts" name="member" id="category">
									...
									<option value="1" <?php if ($tiket->member == '1') {
															echo 'selected';
														} ?>>Member</option>
									<option value="2" <?php if ($tiket->member == '2') {
															echo 'selected';
														} ?>>Non Member</option>
								</select>
