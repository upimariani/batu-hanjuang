...
class cTransaksi extends CI_Controller
{
	...
	public function konfirmasi($id)
	{
		$data_wisatawan = $this->db->query("SELECT * FROM `transaksi` JOIN wisatawan ON transaksi.id_wisatawan=wisatawan.id_wisatawan WHERE transaksi.id_transaksi='" . $id . "'")->row();
		$point_sebelumnya = $data_wisatawan->point;
		$point_persen = (0.4 / 100) * $data_wisatawan->tot_transaksi;
		...
		$this->db->where('id_wisatawan', $data_wisatawan->id_wisatawan);
		$this->db->update('wisatawan', $data_level);
